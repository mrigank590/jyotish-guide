...
const HOUSE_INFO = [
  { h:1,  meaning:"Self, body, personality, general fortune",        keyword:"Self"          },
  { h:2,  meaning:"Wealth, family, speech, accumulated assets",      keyword:"Wealth"        },
  { h:3,  meaning:"Siblings, courage, short journeys, communication",keyword:"Courage"       },
  { h:4,  meaning:"Home, mother, property, emotional foundation",    keyword:"Home"          },
  { h:5,  meaning:"Children, creativity, intelligence, education",   keyword:"Creativity"    },
  { h:6,  meaning:"Enemies, health, service, daily routine, debt",   keyword:"Service"       },
  { h:7,  meaning:"Partner, marriage, business partner, public",     keyword:"Partnership"   },
  { h:8,  meaning:"Longevity, transformation, hidden wealth, in-laws",keyword:"Transformation"},
  { h:9,  meaning:"Luck, dharma, father, higher learning, travel",   keyword:"Dharma"        },
  { h:10, meaning:"Career, status, government, public reputation",   keyword:"Career"        },
  { h:11, meaning:"Gains, networks, elder siblings, income",         keyword:"Gains"         },
  { h:12, meaning:"Loss, liberation, foreign lands, moksha",         keyword:"Liberation"    },
];

const DASHAS = [
  { planet:"Ketu",    years:7,  col:"#8B7355" },
  ...
  { planet:"Rahu",    years:18, col:"#5B6B8A" },
  { planet:"Jupiter", years:16, col:"#C4A84A" },
  { planet:"Saturn",  years:19, col:"#707070" },
...
//
// Standard layout is a 3-column × 4-row grid inside a square.
// The 12 houses occupy these (col, row) positions (0-indexed):
//
//   Row 0:  [H12, col0]  [H1, col1]  [H2, col2]
//   Row 1:  [H11, col0]  [center X]  [H3, col2]
//   Row 2:  [H10, col0]  [H9, col1]  [H4, col2]
//   Row 3:  [H8,  col0]  [H7, col1]  [H5+H6, col2]  ← H5 top-half, H6 bottom-half
//
// The center cell (col1, row1) has decorative X lines — no house number.
// H5 and H6 share the bottom-right cell, split by a horizontal midline.
// Signs rotate clockwise from H1.

const GRID_COLS = [0, 100, 200, 300];
const GRID_ROWS = [0, 75, 150, 225, 300];

// Grid placement for each house (col = left edge index, row = top edge index)
const HOUSE_GRID = {
  12: { col:0, row:0 },
...
// H5 = top half of (col2, row3), H6 = bottom half of (col2, row3)

function getSignForHouse(houseNum, lagnaSign) {
  const offset = (lagnaSign - 1 + houseNum - 1) % 12;
  return SIGNS[offset];
}
...
function KundliChart({ lagnaSign }) {
  ...
  const cellW = (col) => GRID_COLS[col + 1] - GRID_COLS[col];
  const cellH = (row) => GRID_ROWS[row + 1] - GRID_ROWS[row];
  ...
  const houseCells = Object.entries(HOUSE_GRID).map(([h, pos]) => ({
    ...
    w: cellW(pos.col),
    ht: cellH(pos.row),
    isLagna: pos.isLagna || false,
  }));

  // H5 and H6 share col2, row3
  const h56x = GRID_COLS[2];
  const h56y = GRID_ROWS[3];
  const h56w = cellW(2);
  const h56ht = cellH(3);

  // Center cell (col1, row1) coords
  const cX = GRID_COLS[1], cY = GRID_ROWS[1];
  const cW = cellW(1), cHt = cellH(1);
  ...
  return (
    <div style={{ width:"100%", maxWidth:340 }}>
      <svg viewBox="0 0 300 300" style={{ display:"block", width:"100%", border:"2px solid #5B3A29", borderRadius:3, background:"#FFFDF7" }}>
        ...
        {houseCells.map(({ h, x, y, w, ht, isLagna }) => {
          ...
          let bg = "#FFFDF7";
          if (isLagna) bg = isHov ? "#FFE58A" : "#FFF3B0";
          else if (isHov) bg = "#FFF0D0";
          return (
            <g key={h} onMouseEnter={() => setHovered(h)} onMouseLeave={() => setHovered(null)}
               style={{ cursor:"pointer" }}>
              <rect x={x} y={y} width={w} height={ht} fill={bg}
                stroke="#5B3A29" strokeWidth={isLagna ? 1.5 : 1}/>
              <text x={x + w/2} y={y + ht/2 - 7} textAnchor="middle" dominantBaseline="middle"
                fontSize="13" fontWeight="700" fontFamily="Georgia, serif"
                fill={isLagna ? "#8B3A00" : "#3D1F00"}>{h}</text>
              <text x={x + w/2} y={y + ht/2 + 10} textAnchor="middle" dominantBaseline="middle"
                fontSize="9" fontFamily="sans-serif" fill="#9A6030">
                {sign.sym} {sign.num}
              </text>
              {isLagna && (
                <text x={x + w/2} y={y + ht/2 + 22} textAnchor="middle" dominantBaseline="middle"
                  fontSize="7" fontFamily="sans-serif" fill="#C47A30" fontStyle="italic">Lagna</text>
              )}
            </g>
          );
        })}

        {/* H5 — top half of bottom-right cell */}
        {[5, 6].map((h) => {
          const isTop = h === 5;
          ...
          return (
            <g key={h} onMouseEnter={() => setHovered(h)} onMouseLeave={() => setHovered(null)}
              ...
              <rect x={h56x} y={y} width={h56w} height={ht}
                fill={isHov ? "#FFF0D0" : "#FFFDF7"} stroke="#5B3A29" strokeWidth="1"/>
              <text x={h56x + h56w/2} y={y + ht/2 - 5} textAnchor="middle" dominantBaseline="middle"
                fontSize="11" fontWeight="700" fontFamily="Georgia, serif" fill="#3D1F00">{h}</text>
              <text x={h56x + h56w/2} y={y + ht/2 + 7} textAnchor="middle" dominantBaseline="middle"
                fontSize="8" fontFamily="sans-serif" fill="#9A6030">
                {sign.sym} {sign.num}
              </text>
            </g>
          );
        ...
        {/* Center decorative cell with X */}
        <rect x={cX} y={cY} width={cW} height={cHt} fill="#FFF8EE" stroke="#5B3A29" strokeWidth="1"/>
        <line x1={cX} y1={cY} x2={cX+cW} y2={cY+cHt} stroke="#C4933A" strokeWidth="1" opacity="0.5"/>
        <line x1={cX+cW} y1={cY} x2={cX} y2={cY+cHt} stroke="#C4933A" strokeWidth="1" opacity="0.5"/>

        {/* Outer border on top */}
        <rect x="0" y="0" width="300" height="300" fill="none" stroke="#5B3A29" strokeWidth="2"/>
      </svg>

      {/* Tooltip strip */}
      <div style={{ minHeight:54, marginTop:6, padding:"8px 12px", background:"#FFF8EE",
        border:"1px solid #D4A050", borderRadius:3, fontSize:12, fontFamily:"sans-serif", color:"#5B3A29" }}>
        {hoverInfo ? (
          <>
            <span style={{ fontWeight:700, color:"#3D1F00" }}>House {hovered}</span>
            {" — "}
            <span style={{ fontWeight:600, color:"#8B3A00" }}>{hoverSign?.name}</span>
            {" "}
            <span style={{ color:"#9A7050" }}>({hoverSign?.hindi})</span>
            <div style={{ marginTop:3, color:"#5B3A29", lineHeight:1.4 }}>{hoverInfo.meaning}</div>
          </>
        ) : (
          <span style={{ color:"#B09070", fontStyle:"italic" }}>Hover a house to see its meaning and sign</span>
        )}
      </div>
    </div>
  );
...
function Sec({ id, title, children }) {
  return (
    <section id={id} style={{ marginBottom:48, scrollMarginTop:20 }}>
      <h2 style={{ fontSize:19, fontWeight:700, color:C.head, borderBottom:`2px solid ${C.border}`,
        paddingBottom:8, marginBottom:20, fontFamily:"Georgia,serif", letterSpacing:"0.01em" }}>
        {title}
      </h2>
      {children}
    </section>
...
function H3({ children }) {
  return <h3 style={{ fontSize:14, fontWeight:700, color:"#5B3A00", marginTop:22, marginBottom:10,
    fontFamily:"sans-serif", textTransform:"uppercase", letterSpacing:"0.06em" }}>{children}</h3>;
}

function P({ children }) {
  return <p style={{ fontSize:13, lineHeight:1.75, color:C.ink, marginBottom:14,
    fontFamily:"sans-serif" }}>{children}</p>;
}
  ...
  return (
    <div style={{ overflowX:"auto", marginBottom:16 }}>
      <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12, fontFamily:"sans-serif" }}>
        <thead>
          <tr>{headers.map(h => (
            <th key={h} style={{ padding:"7px 12px", background:C.head, color:"#FFFDF7",
        ...
        <tbody>
          {rows.map((row, i) => {
            ...
            return (
              <tr key={i} style={{ background: isHi ? C.hi : i%2===0 ? C.card : "#FFFDF7" }}>
                {row.map((cell, j) => (
                  <td key={j} style={{ padding:"6px 12px", borderBottom:`1px solid #E8D4A0`,
                    color: C.ink, fontWeight: isHi && j===0 ? 700 : 400 }}>{cell}</td>
                ))}
              </tr>
            );
          })}
  ...
  return (
    <div style={{ background: warn ? C.warnBg : C.card, border:`1px solid ${warn ? C.warnBd : C.border}`,
      ...
      <div style={{ fontWeight:700, fontSize:13, fontFamily:"sans-serif",
        color: warn ? C.warn : "#5B3A00", marginBottom:7 }}>{title}</div>
      <ul style={{ margin:0, paddingLeft:18 }}>
        {items.map((item, i) => (
          <li key={i} style={{ marginBottom:5, color: warn ? "#6A3020" : C.mid,
    ...
    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:5,
      ...
      <div style={{ display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:6, marginBottom:6 }}>
        <div>
          ...
          {badge && <span style={{ marginLeft:8, fontSize:10, fontFamily:"sans-serif", fontWeight:700,
            padding:"2px 7px", borderRadius:10, background: badge==="Free"?"#E8F5E9":"#FFF3E0",
            color: badge==="Free"?"#2E7D32":"#E65100",
            border:`1px solid ${badge==="Free"?"#A5D6A7":"#FFCC80"}` }}>{badge}</span>}
          {sub && <span style={{ marginLeft:8, fontSize:11, color:C.muted, fontFamily:"sans-serif" }}>{sub}</span>}
        </div>
        {rating && <span style={{ color:"#D4A050", letterSpacing:1, fontSize:13 }}>{"★".repeat(rating)}{"☆".repeat(5-rating)}</span>}
      </div>
      {url && <div style={{ fontSize:11, color:C.muted, fontFamily:"sans-serif", marginBottom:6 }}>↗ {url}</div>}
      {(pros || cons) && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginTop:6 }}>
          {pros && <div>
            <div style={{ fontSize:10, fontWeight:700, color:"#2E7D32", marginBottom:4, fontFamily:"sans-serif" }}>✓ Strengths</div>
            <ul style={{ margin:0, paddingLeft:14 }}>
              {pros.map((p,i) => <li key={i} style={{ fontSize:11, color:C.ink, lineHeight:1.6, fontFamily:"sans-serif" }}>{p}</li>)}
            </ul>
          </div>}
          {cons && <div>
            <div style={{ fontSize:10, fontWeight:700, color:"#B71C1C", marginBottom:4, fontFamily:"sans-serif" }}>⚠ Watch out</div>
            <ul style={{ margin:0, paddingLeft:14 }}>
...
function WeekRow({ label, text }) {
  return (
    <div style={{ display:"flex", gap:10, marginBottom:10 }}>
      <div style={{ minWidth:72, background:C.head, color:"#FFFDF7", borderRadius:3,
    ...
    <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(230px,1fr))", gap:10, marginBottom:16 }}>
      {items.map(({ title, desc }) => (
        <div key={title} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:4, padding:"10px 12px" }}>
          <div style={{ fontWeight:700, fontSize:12, fontFamily:"sans-serif", color:"#5B3A00", marginBottom:4 }}>{title}</div>
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily:"sans-serif", background:C.bg, minHeight:"100vh", color:C.ink }}>
      ...
      {/* Header */}
      <header style={{ background:C.head, color:"#FFFDF7", padding:"16px 24px",
        position:"sticky", top:0, zIndex:200, boxShadow:"0 2px 10px rgba(0,0,0,0.35)",
        display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div>
          <div style={{ fontSize:17, fontWeight:700, fontFamily:"Georgia,serif", letterSpacing:"0.02em" }}>
            ✦ Jyotish Study Guide
          </div>
          <div style={{ fontSize:11, color:"#D4A050", marginTop:2, fontStyle:"italic" }}>
            Vedic Astrology · North Indian Kundli · Mahadasha & Antardasha
          </div>
        </div>
        <button onClick={() => setMenuOpen(o => !o)}
          style={{ display:"none", background:"none", border:`1px solid #D4A050`, color:"#D4A050",
            padding:"5px 11px", borderRadius:3, cursor:"pointer", fontSize:12 }}
      ...
      <div style={{ display:"flex", maxWidth:1080, margin:"0 auto", padding:"0 12px" }}>
        ...
        {/* Sidebar */}
        <nav style={{ width:196, minWidth:196, position:"sticky", top:55, height:"calc(100vh - 55px)",
          overflowY:"auto", padding:"18px 0", borderRight:`1px solid ${C.border}`, flexShrink:0 }}
          className="sidebar">
          ...
          {NAV.map(s => (
            <button key={s.id} onClick={() => goto(s.id)}
              style={{ display:"block", width:"100%", textAlign:"left", background:"none",
                ...
                color: active===s.id ? C.lagna : C.mid,
                fontWeight: active===s.id ? 700 : 400,
                borderLeft: active===s.id ? `3px solid ${C.border}` : "3px solid transparent" }}>
              {s.label}
            </button>
          ))}
        </nav>
        ...
        <main style={{ flex:1, padding:"26px 28px 80px", minWidth:0 }}>
          ...
          <Sec id="overview" title="What You're Actually Learning">
            <P>
              Vedic astrology (Jyotish = "light/knowledge") uses name, birth date, exact birth time, and birth place
              to construct a detailed map of tendencies, timing, and life areas.
            </P>
            <DataTable
            ...
            <DataTable
              ...
              rows={[
                ...
                ["Moon importance", "Moon sign is critical — represents mind and emotions", "Sun sign most emphasised"],
                ["Timing system", "Dasha system (Mahadasha / Antardasha)", "Transits, progressions, solar returns"],
                ["Extra bodies", "Rahu & Ketu (nodes) used prominently in predictions", "Includes Uranus/Neptune/Pluto; nodes less central"],
            ...
            />
            <P>
              If you see Westerners talking about "natal chart," "houses," "conjunctions," and "aspects" — they mean
              very similar things. The technical framework and emphasis differ significantly, but the vocabulary overlaps.
            </P>
          </Sec>
          ...
          <Sec id="kundli" title="North Indian Kundli — Chart Layout">
            <P>
              This guide focuses on the <strong>North Indian (Uttar Bharatiya) style</strong> — the standard across
              North India, Delhi, UP, MP, Rajasthan, Punjab, Gujarat, Maharashtra, and most Hindi-language books.
              It is also the default in JHora software and AstroSage.
            </P>
            ...
            <div style={{ display:"flex", gap:28, flexWrap:"wrap", alignItems:"flex-start", marginBottom:24 }}>
              ...
              <div style={{ flexShrink:0 }}>
                <div style={{ marginBottom:8, fontSize:12, fontWeight:700, color:"#5B3A00" }}>
            ...
            <P>These six pairs sit directly across from each other. Every planet aspects the house opposite — the basis of the universal 7th-house aspect.</P>
            ...
            <div style={{ background:"#F5EED8", border:`1px solid ${C.hiBd}`, borderRadius:4,
              padding:"10px 14px", fontSize:12, color:"#5B3A00" }}>
          ...
          <Sec id="foundations" title="Phase 1 — Absolute Foundations">
            ...
            <H3>Planet abbreviations used in North Indian charts</H3>
          ...
          <Sec id="charts" title="Phase 2 — Reading the D1 Birth Chart">
            ...
            {[
              ...
              <div key={title} style={{ display:"flex", gap:12, marginBottom:10, padding:"8px 12px",
                ...
                <div style={{ minWidth:160, fontWeight:700, fontSize:12, color:"#5B3A00", lineHeight:1.5 }}>{title}</div>
          ...
          <Sec id="dasha" title="Phase 3 — Dasha System (Mahadasha & Antardasha)">
            <P>
              This is the heart of Jyotish timing predictions. The Vimshottari Dasha is a 120-year planetary
              timeline triggered by the Moon's Nakshatra at birth. Learn this before any other dasha system.
            </P>
            ...
            <div style={{ background:C.hi, border:`1px solid ${C.hiBd}`, borderRadius:4,
              padding:"10px 14px", marginBottom:16, fontSize:12, color:"#5B3A00" }}>
              <strong>How the starting Dasha is determined:</strong> The Nakshatra the Moon occupied at birth
              determines which Dasha you're born into and how much of it remains. This is why exact birth time
              matters — even a few minutes can shift the Nakshatra.
            </div>
            ...
            <GridCards items={[
              ...
              { title:"Char Dasha (Jaimini)", desc:"Sign-based rather than planet-based. Highly respected for event timing. Requires learning Jaimini's separate framework." },
              { title:"Kalachakra Dasha",     desc:"Based on Moon's Nakshatra pada. Considered very accurate but more complex to calculate." },
            ]} />
          </Sec>
          ...
          <Sec id="vargas" title="Phase 4 — Divisional Charts (Vargas)">
            <P>
              The 16 main divisional charts (Shodasha Vargas) mathematically divide each sign into smaller
              parts to reveal specifics about individual life areas. Start with D9 and D10 before the others.
            </P>
            <DataTable
          ...
          <Sec id="advanced" title="Phase 5 — Advanced Topics">
            ...
            <GridCards items={[
              { title:"Ashtakavarga",           desc:"A numerical scoring system (0–8 per house) for evaluating transit strength. Very practical for predicting good/bad transit periods." },
              { title:"Jaimini Astrology",      desc:"A parallel system using Char Dasha and Karakas (7 special planet roles). Highly respected for accuracy in event timing." },
              { title:"KP Astrology",           desc:"Krishnamurti Paddhati — uses sublord theory. Very popular in South India for event timing. Note: contradicts some classical Parashari rules." },
              { title:"Nadi Astrology",         desc:"Reading from ancient palm leaf manuscripts. Requires knowing your Naadi Amsha. A very different approach from chart-based methods." },
              { title:"Prashna (Horary)",       desc:"Answering a question based on the exact moment it is asked, without needing a birth chart." },
              { title:"Muhurta",                desc:"Selecting auspicious timing for events — marriage, business launches, surgeries, property purchase." },
              { title:"Varshaphala",            desc:"The annual solar return chart. Analysed each year to understand that year's major themes." },
              { title:"Sade Sati",              desc:"Saturn's ~7.5-year transit across 3 signs including the Moon sign. Considered a period of testing, restructuring, and transformation." },
            ]} />
          </Sec>
          ...
          <Sec id="youtube" title="YouTube Channels">
            ...
            <ResourceCard name="KRSchannel (Kapiel Raaj)" rating={3}
              ...
              cons={["Widely criticised for oversimplification and inaccuracies against classical texts","Mixes Western and Vedic ideas without flagging the distinction","Treat as a starting point only — always verify with classical sources"]} />
            ...
            {[
              { name:"Komilla Sutton (BAVA)", rating:5, desc:"One of the most respected Western Jyotish teachers. Excellent for Nakshatras and structured learning. Much free content on YouTube; paid subscription platform for deeper material." },
              ...
              { name:"Sanjay Rath", rating:4, desc:"One of the most respected living Jyotishis globally. Deep classical knowledge in both Jaimini and Parashari. Content is dense and assumes prior knowledge — not for beginners." },
              { name:"Hindi-language channels", rating:4, desc:"Dozens of excellent free channels in Hindi. Search: 'Kundli parhna seekho' or 'jyotish ke mool siddhant' for deep dives." },
            ].map(ch => (
              <div key={ch.name} style={{ background:C.card, border:`1px solid ${C.border}`,
                ...
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                  ...
                  <span style={{ color:"#D4A050", letterSpacing:1 }}>{"★".repeat(ch.rating)}{"☆".repeat(5-ch.rating)}</span>
                </div>
          ...
          <Sec id="books" title="Books">
            ...
            {[
              { title:"Astrology for Beginners", author:"B.V. Raman", where:"Free at archive.org", badge:"Free", rating:5,
                note:"The classic Indian starting point. Clear, structured, with examples. Raman is considered one of the 20th century's greatest Jyotishis." },
              { title:"Fundamentals of Vedic Astrology", author:"Bepin Behari", where:"Free PDF", badge:"Free", rating:4,
                note:"Combines spiritual and technical. Good for understanding why the rules exist, not just what they are." },
              { title:"PVR Narasimha Rao's Free E-book", author:"PVR Narasimha Rao", where:"vedicastrologer.org", badge:"Free", rating:5,
                note:"Concise, modern, technically careful. Best free text for learning software-aided Jyotish." },
              { title:"Brihat Parashara Hora Shastra (BPHS)", author:"Attributed to Sage Parashara", where:"archive.org", badge:"Free", rating:5,
                note:"The root classical text. Not beginner-friendly but invaluable as a reference once you know the basics." },
            ].map(b => <ResourceCard key={b.title} name={b.title} badge={b.badge} sub={`${b.author} · ${b.where}`} rating={b.rating}
              pros={[b.note]} />)}

            <H3>Paid books worth buying</H3>
            {[
              { title:"The Art and Science of Vedic Astrology (Vol. 1 & 2)", author:"Ryan Kurczak & Richard Fish", price:"~₹1,500–2,000 / $20–25", rating:5,
                note:"The most recommended beginner-to-intermediate textbook. Structured like a course. Goodreads: 4.27/5." },
              { title:"The Essentials of Vedic Astrology", author:"Komilla Sutton", price:"~₹1,200–1,800", rating:5,
                note:"Excellent for signs, houses, planets, nakshatras. Goodreads: 4.31/5. Well-loved by both Indian and Western students." },
              { title:"Light on Life", author:"Hart de Fouw & Robert Svoboda", price:"~₹2,000–3,000", rating:5,
                note:"The most comprehensive English-language Jyotish textbook. Dense but thorough. Best for serious students." },
              { title:"Astrology of the Seers", author:"David Frawley", price:"~₹1,000–1,500", rating:4,
                note:"A well-respected bridge between Vedic philosophy and astrology; very readable." },
              { title:"Path of Light (Vol. 1 & 2)", author:"James Kelleher", price:"~₹2,500–3,500", rating:4,
                note:"Deep treatment of Nakshatras with traditional artwork. Praised by David Frawley." },
              { title:"Crux of Vedic Astrology: Timing of Events", author:"Sanjay Rath", price:"~₹2,000–3,000", rating:5,
                note:"Best book in English on multiple dasha systems and event timing. For intermediate+ students only." },
              { title:"Learn Think and Predict Through Astrology", author:"Prof. Arora", price:"~₹400–600", rating:3,
                note:"Budget-friendly, practical Indian perspective. 60% five-star reviews on Goodreads." },
            ].map(b => <ResourceCard key={b.title} name={b.title} badge="Paid" sub={`${b.author} · ${b.price}`} rating={b.rating}
              pros={[b.note]} />)}
          </Sec>
          ...
          <Sec id="courses" title="Paid Courses Worth Considering">
            ...
            {[
              { name:"Asheville Vedic Astrology — Ryan Kurczak", url:"ashevillevedicastrology.com", price:"~$42 per topic course",
                note:"Audio/video download courses. Navamsha + Dashamsha course is particularly good. Structured, classical, best Western-produced paid content available." },
              { name:"Komilla Sutton's School", url:"komillasutton.com", price:"Expensive — certificate programs",
                note:"Certificate programs. Thorough for those wanting a formal credential." },
              { name:"Sanjay Rath's Courses (SJC)", url:"sjc.org.in", price:"Varies",
                note:"For advanced students only. Highly technical and traditional. Not beginner-appropriate." },
              { name:"Udemy — Kundli Mastery (Astro Arun Pandit)", url:"udemy.com", price:"~₹500–1,500 on sale",
                note:"Covers foundations through Mahadasha/Antardasha with case studies. Check recent reviews before buying — Udemy course quality varies." },
              { name:"BAVA (Bharat Astro Veda Academy)", url:"bharatastroveda.com", price:"Varies by course",
                note:"Live weekly classes with Acharya Amit Garg. Covers Nakshatras, Dasha systems, D9/D10. 12 months access to recordings. Requires Phase 1 completion." },
            ].map(c => (
              <div key={c.name} style={{ background:C.card, border:`1px solid ${C.border}`,
                borderRadius:4, padding:"10px 14px", marginBottom:10 }}>
                ...
                <div style={{ fontSize:11, color:C.muted, margin:"2px 0 5px" }}>
                  <span style={{ marginRight:8, padding:"2px 7px", borderRadius:10, fontSize:10, fontWeight:700,
                    background:"#FFF3E0", color:"#E65100", border:"1px solid #FFCC80" }}>Paid</span>
                  {c.url} · {c.price}
                </div>
          ...
          <Sec id="community" title="Communities to Join">
            <GridCards items={[
              ...
              { title:"r/astrology (Reddit)",               desc:"Larger but mostly Western. Useful for comparison and understanding vocabulary overlap." },
              ...
              { title:"LightOnVedicAstrology.com",          desc:"Older phpBB forum but a gold mine of serious discussion. Has a well-maintained book recommendation thread." },
              ...
              { title:"Jyotiṣa by Pt. Katti Narahari (Substack)", desc:"1,000+ subscribers. Traditional approach. Free tier available." },
            ]} />
          </Sec>
          ...
          <Sec id="redflags" title="⚠ Red Flags & Honest Negatives">
            ...
            <Card warn title="About the subject itself" items={[
              "No scientific validation — astrology as a predictive system has not passed controlled double-blind studies. Decide for yourself how you want to engage with it.",
              "Interpretation varies wildly — two experienced Jyotishis can read the same chart and give completely different predictions. There is no unified 'correct' interpretation.",
              "Birth time dependency — errors of even 2–4 minutes can change the Lagna, shift Navamsha placements, and alter Dasha start times. Hospital-recorded times in India are often approximate.",
            ...
            <Card warn title="About specific resources" items={[
              "KRSchannel: widely popular but criticised by serious practitioners for oversimplification and mixing Western/Vedic ideas without flagging it. Good to start; verify against classical sources.",
              "Paid consultation platforms (AstroTalk, ClickAstro, etc.): ClickAstro has a 2.3/5 Trustpilot average with multiple complaints about vague auto-generated reports and upselling.",
              "Computerised Kundli reports from any site: auto-generated text applies cookbook interpretations without understanding the full chart as a system. Learn to read charts yourself.",
              ...
              "'Free' courses that funnel to consultations: many Indian YouTube channels use free content to push expensive one-on-one sessions. Watch the free content; be cautious about paid funnels.",
            ]} />

            <Card warn title="About the learning process" items={[
              "It takes years, not weeks — serious practitioners say 3–5 years of dedicated study before reading charts with real confidence. Anyone promising quick mastery is overselling.",
              "The 'guru problem' — traditional Jyotish was transmitted guru-to-student. Self-study can take you far, but some classical interpretations are genuinely unclear without a teacher.",
              "Different schools disagree on basics — Parashari vs Jaimini differ significantly. KP astrology contradicts some classical Parashari rules. Know which tradition you're studying.",
            ]} />
          </Sec>
          ...
          <Sec id="studyplan" title="Suggested Weekly Study Plan">
            ...
            <WeekRow label="Weeks 5–6" text="Learn planetary aspects (Drishti). Practice on your own chart and 2–3 family members' charts. Focus on which houses each planet aspects." />
            <WeekRow label="Week 7" text="Study Yogas — start with Raj Yoga and Dhana Yoga. Watch KRS or Ryan Kurczak videos on specific Yogas and verify rules against classical sources." />
            <WeekRow label="Week 8" text="Learn Nakshatras. Read Komilla Sutton's Essentials or watch her YouTube videos. This is a long subject — treat this week as an introduction." />
            ...
            <WeekRow label="Week 16" text="Join r/vedicastrology and practice reading other people's charts for community feedback. Post your own chart for input." />

            <div style={{ background:C.head, color:"#FFFDF7", borderRadius:5,
              padding:"14px 18px", marginTop:20, fontSize:12, lineHeight:1.75 }}>
              <div style={{ fontWeight:700, fontSize:13, color:"#D4A050", marginBottom:8 }}>Ongoing habits</div>
              <ul style={{ margin:0, paddingLeft:18 }}>
                {[
            ...
            <div style={{ marginTop:24, padding:"12px 16px", border:`1px solid ${C.border}`,
              borderRadius:4, fontSize:12, fontStyle:"italic", color:C.muted, lineHeight:1.75,
              fontFamily:"Georgia,serif" }}>
              Jyotish is vast — even professional astrologers with decades of experience describe themselves
              as perpetual students. Progress over perfection.
            </div>
          </Sec>
      ...
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        ::-webkit-scrollbar-thumb { background: #D4A050; border-radius: 3px; }
